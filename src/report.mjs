...
export function writeReport(runId, status) {
  ...
  const lines = [
    ...
    `- **state:** ${status.state}`,
    ...(status.correctionResolution ? [`- **superseded by accepted correction:** ${status.correctionResolution.childRunId}`] : []),
    `- **engine:** ${status.agentManager?.version || "unknown"} · commit ${status.agentManager?.commit || "unrecorded"} · dirty ${status.agentManager?.dirty ?? "unrecorded"}`,
    ...(status.delivery?.review?.family ? [`- **correction family:** ${status.delivery.review.family.rootRunId} · ${status.delivery.review.family.consumedCorrections} corrections consumed before this run`] : []),
